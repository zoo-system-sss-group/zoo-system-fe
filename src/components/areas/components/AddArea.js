...
function AddArea({ fetch }) {
	...
	const saveNewArea = () => {
		...
		axios
			...
			.catch((err) => {
				console.log(err);
				return setErrorMessage(err.response.data.value + " (Maybe your code already exist)");
			});
	};
		...
		<>
			<div className="inline-block float-right">
				<button
					...
					onClick={() => document.getElementById("addArea").showModal()}
				>
					Add New
				</button>
				<dialog id="addArea" className="modal ">
