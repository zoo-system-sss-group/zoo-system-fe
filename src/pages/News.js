	...
	return (
		<div>
			...
			<GuestLayout title="All news" className={"min-h-[400px]"}>
				<div
					className={
						"mx-8 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 relative justify-center gap-8"
					}
				>
					{loading ? (
						<>
						...
						news.map((newObj) => (
							<Link
								to={`${newObj.id}`}
								key={newObj.id}
								className="card lg:w-[400px] w-[300px] mx-auto bg-cor4 shadow-md shadow-cor5 hover:scale-[1.025] cursor-pointer hover:transition"
								title={newObj.title}
							>
								<figure>
									<img
